...
begin
    ...
    for i in 1 .. 4 loop
        v_item_id := pk_util_parallel_execute.add_item_to_task(p_task_name_in => v_task_name, p_plsql_block_in => 'begin dbms_lock.sleep(5); end;');
    end loop;
...
where task_name = 'test156'
order by chunk_id
;

select * from parallel_tasks t where t.task_name = 'test146' ;
select * from parallel_task_items i where i.task_name = 'test146' ;  
